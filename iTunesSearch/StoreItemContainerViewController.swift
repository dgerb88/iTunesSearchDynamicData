...
class StoreItemContainerViewController: UIViewController, UISearchResultsUpdating {
    ...
    func configureTableViewDataSource(_ tableView: UITableView) {
        tableViewDataSource = UITableViewDiffableDataSource<String, StoreItem>(tableView: tableView, cellProvider: { tableView, indexPath, item in
    ...
    @objc func fetchMatchingItems() {
        ...
        searchTask = Task {
            if !searchTerm.isEmpty {
                ...
                await collectionViewDataSource.apply(itemSnapshot)
            } else {
                // apply data source changes
                await tableViewDataSource.apply(itemSnapshot)
                await collectionViewDataSource.apply(itemSnapshot)
